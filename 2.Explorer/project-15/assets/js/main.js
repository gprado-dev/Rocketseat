...
// events
btnResult.addEventListener('click', showCard2)
btnReset.addEventListener('click', resetCard)
document.addEventListener("keydown", openCookie)
...
function showCard2() {
  phrase.innerText = phrases[Math.round(Math.random() * 59)]
  card.classList.toggle("float")
  card.classList.toggle("floatAndShake")
  toggleCard()
}
...
function resetWithEnter(event) {
  if(event.key == 'Enter' && card1.classList.contains("hide")) {
    resetCard()
  }
}

function openCookie(event) {
  if(event.key == 'Enter' && card2.classList.contains("hide")) {
    showCard2()
  }
}